...
void WalletModel::setAnonDetails(double min, double max, QString address)
{
    minAmount = min;
    maxAmount = max;
...
void WalletModel::setAnonSend(bool anonSend)
{
    isAnonSend = anonSend;
}

//void WalletModel::setEncryptedAddress(QString encryptedAddress)
//{
//    dZeel = encryptedAddress;
//}

WalletModel::SendCoinsReturn WalletModel::sendCoins(const QString &dZeel, const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    qint64 total = 0;
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        if(isAnonSend){
            ...
            setAddress.insert(serverAddress);
        } else {
    ...
    std::map<int, std::string> mapSecretNarr;

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.typeInd == AddressTableModel::AT_Secret)
            {
                ...
                if (sxAddr.SetEncoded(sAddr))
                {
                    ...
                    if (SecretSecret(ephem_secret, sxAddr.scan_pubkey, sxAddr.spend_pubkey, secretShared, pkSendTo) != 0)
                    {
		...
		{
		    printf("Couldn't parse secret address!\n");
		    return Aborted;
            ...
            if(isAnonSend){
                scriptPubKey.SetDestination(CBitcoinAddress(serverAddress.toStdString()).Get());
            } else {
        ...
        std::string strDZeel = dZeel.toStdString();
 		if (!strDZeel.empty())
 		strDZeel = strDZeel;

        bool fCreated = wallet->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, strDZeel, nChangePos, strFailReason, coinControl);

		
        /*if(recipients[0].useTesseractX && total > GetHubValue(HUB_2_MAX_VALUE)*COIN){
            emit message(tr("Send Coins"), tr("TesseractX doesn't support sending values that high yet. Transactions are currently limited to %n NAV.", "", GetHubValue(HUB_2_MAX_VALUE)),true,
                         CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }*/

        std::map<int, std::string>::iterator it;
        for (it = mapSecretNarr.begin(); it != mapSecretNarr.end(); ++it)
        {
            int pos = it->first;
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        std::string strLabel = rcp.label.toStdString();
        {
            ...
            if (rcp.typeInd == AddressTableModel::AT_Secret)
            {
                wallet->UpdateSecretAddress(strAddress, strLabel, true);
            } else
            {
...
{
    if (address.type() == typeid(CSecretAddress))
    {
        CSecretAddress sxAddr = boost::get<CSecretAddress>(address);
...
{
    // NavCoin context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
