...
bool CWallet::Lock()
{
    ...
    
    {
        ...
        // -- load encrypted spend_secret of secret addresses
        CSecretAddress sxAddrTemp;
        std::set<CSecretAddress>::iterator it;
        for (it = secretAddresses.begin(); it != secretAddresses.end(); ++it)
        {
            if (it->scan_secret.size() < 32)
                continue; // secret address is not owned
            // -- CSecretAddress are only sorted on spend_pubkey
            CSecretAddress &sxAddr = const_cast<CSecretAddress&>(*it);
            if (fDebug)
                printf("Recrypting secret key %s\n", sxAddr.Encoded().c_str());
            
            sxAddrTemp.scan_pubkey = sxAddr.scan_pubkey;
            if (!wdb.ReadSecretAddress(sxAddrTemp))
            {
                printf("Error: Failed to read secret key from db %s\n", sxAddr.Encoded().c_str());
                continue;
            }
...
bool CWallet::Unlock(const SecureString& strWalletPassphrase, bool anonymizeOnly)
{
	...
	fWalletUnlockAnonymizeOnly = anonymizeOnly;
	UnlockSecretAddresses(vMasterKey);
	return true;
    }
    return false;
}
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        for (it = secretAddresses.begin(); it != secretAddresses.end(); ++it)
        {
            if (it->scan_secret.size() < 32)
                continue; // secret address is not owned
            // -- CSecretAddress is only sorted on spend_pubkey
            CSecretAddress &sxAddr = const_cast<CSecretAddress&>(*it);
            
            if (fDebug)
                printf("Encrypting secret key %s\n", sxAddr.Encoded().c_str());
            ...
            {
                printf("Error: Failed encrypting secret key %s\n", sxAddr.Encoded().c_str());
                continue;
            };
            
            sxAddr.spend_secret = vchCryptedSecret;
            pwalletdbEncryption->WriteSecretAddress(sxAddr);
        };
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %s NAV %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransaction& tx, const CBlock* pblock, bool fUpdate)
{
    uint256 hash = tx.GetHash();
    {
        ...
        mapValue_t mapNarr;
        FindSecretTransactions(tx, mapNarr);
...
bool CWallet::IsChange(const CTxOut& txout) const
{
    ...
    // payment to a NAV_PUBKEYHASH that is mine but isn't in the address book
                           ...
                           list<pair<CTxDestination, int64_t> >& listSent, int64_t& nFee, string& strSentAccount) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
	    ...
	    // NAVNOTE: CoinControl possible fix related... with HD wallet we need to report change?
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s NAV %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
		    ...
		    if(pcoin->IsSpent(i) || !IsMine(pcoin->vout[i]) || !IsDenominated(vin)) continue;
                    ...
                    int rounds = GetInputAnonsendRounds(vin);
                    nTotal += pcoin->vout[i].nValue * rounds / nAnonsendRounds;
                }
            }
        }
    }
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, AvailableCoinsType coin_type, bool useTSX) const
{
    vCoins.clear();

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            int nDepth = pcoin->GetDepthInMainChain();
            if (nDepth <= 0) // NAVNOTE: coincontrol fix / ignore 0 confirm 
                continue;
           ...
           /* for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            ...
            // do not use TSX for inputs that have less then 6 blockchain confirmations
            if (useTSX && nDepth < 6)
                continue;
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                } else if(coin_type == ONLY_NONDENOMINATED || coin_type == ONLY_NONDENOMINATED_NOTIN) {
                    found = true;
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && coin_type == ONLY_NONDENOMINATED_NOTIN) found = (pcoin->vout[i].nValue != 100000*COIN); // do not use funds
                } else {
                    found = true;
                }
                if(!found) continue;
...
void CWallet::AvailableCoinsIN(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, AvailableCoinsType coin_type, bool useTSX) const
{
    vCoins.clear();
...
bool less_then_denom (const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH(int64_t d, anonSendDenominations) // loop through predefined denoms
    {
...
bool CWallet::SelectCoins(int64_t nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useTSX) const
{
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl);

    //if we're doing only denominated, we need to round up to the nearest .1NAV
    if(coin_type == ONLY_DENOMINATED){
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH(int64_t v, anonSendDenominations)
        {
            int added = 0;
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*COIN)+100 //round the amount up to .1NAV over
                    && added <= 100){                                                          //don't add more than 100 of one denom type
                        CTxIn vin = CTxIn(out.tx->GetHash(),out.i);
                        int rounds = GetInputAnonsendRounds(vin);
                        // make sure it's actually anonymized
                        if(rounds < nAnonsendRounds) continue;
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, vector<COutput>& setCoinsRet2, int64_t& nValueRet, int nAnonsendRoundsMin, int nAnonsendRoundsMax)
{
    setCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fINode && out.tx->vout[out.i].nValue == 100000*COIN) continue; //inode input
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            int rounds = GetInputAnonsendRounds(vin);
            if(rounds >= nAnonsendRoundsMax) continue;
            if(rounds < nAnonsendRoundsMin) continue;
...
bool CWallet::SelectCoinsAnon(int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, int64_t& nValueRet, int nAnonsendRoundsMin, int nAnonsendRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nAnonsendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOTIN : ONLY_DENOMINATED);
...
{
    return  nInputAmount == (ANONSEND_COLLATERAL * 5)+ANONSEND_FEE ||
            nInputAmount == (ANONSEND_COLLATERAL * 4)+ANONSEND_FEE ||
            nInputAmount == (ANONSEND_COLLATERAL * 3)+ANONSEND_FEE ||
            nInputAmount == (ANONSEND_COLLATERAL * 2)+ANONSEND_FEE ||
            nInputAmount == (ANONSEND_COLLATERAL * 1)+ANONSEND_FEE;
}
...
bool CWallet::CreateCollateralTransaction(CTransaction& txCollateral, std::string strReason)
{
    ...
    */
    ...
    {
        strReason = "Error: Anonsend requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    if(nValueIn2 - ANONSEND_COLLATERAL - nFeeRet > 0) {
        //pay collateral charge in fees
        CTxOut vout3 = CTxOut(nValueIn2 - ANONSEND_COLLATERAL, scriptChange);
    ...
    BOOST_FOREACH(CTxIn v, txCollateral.vin) {
        if(!SignSignature(*this, v.prevPubKey, txCollateral, vinNumber, int(SIGHASH_ALL|SIGHASH_ANYONECANPAY))) {
            ...
            strReason = "CAnonSendPool::Sign - Unable to sign collateral transaction! \n";
            return false;
        }
        vinNumber++;
    }
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, std::string strDZeel, int32_t& nChangePos, std::string& strFailReason, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useTSX)
{
    ...
    wtxNew.strDZeel = strDZeel;
     ...
     if (wtxNew.strDZeel.length() > 0)
         wtxNew.nVersion = CTransaction::TXDZEEL_VERSION;	 
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl))
                {
                    ...
                    } else if (coin_type == ONLY_NONDENOMINATED_NOTIN) {
                        strFailReason = _("Unable to locate enough Anonsend non-denominated funds for this transaction that are not equal 1000 NAV.");
                    } else {
                        strFailReason = _("Unable to locate enough Anonsend denominated funds for this transaction.");
                        strFailReason += _("Anonsend uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                    }

                    if(useTSX){
                        strFailReason += _("TesseractX requires inputs with at least 6 confirmations, you might need to wait a few minutes and try again.");
                    }
                    return false;
                }
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, std::string strDZeel, const CCoinControl* coinControl)
{
    ...
    bool rv = CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, strDZeel, nChangePos, strFailReason, coinControl);
    
    // -- narration will be added to mapValue later in FindSecretTransactions From CommitTransaction
    return rv;
}


bool CWallet::NewSecretAddress(std::string& sError, std::string& sLabel, CSecretAddress& sxAddr)
{
        ...
        || GenerateRandomSecret(spend_secret) != 0)
    {
        ...
        printf("Error CWallet::NewSecretAddress - %s\n", sError.c_str());
        return false;
    };
...
bool CWallet::AddSecretAddress(CSecretAddress& sxAddr)
{
    LOCK(cs_wallet);
    
    // must add before changing spend_secret
    secretAddresses.insert(sxAddr);
    ...
    if (fOwned)
    {
        ...
        if (IsLocked())
        {
            printf("Error: CWallet::AddSecretAddress wallet must be unlocked.\n");
            secretAddresses.erase(sxAddr);
            return false;
        };
    ...
    bool rv = CWalletDB(strWalletFile).WriteSecretAddress(sxAddr);
...
bool CWallet::UnlockSecretAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    // -- decrypt spend_secret of secret addresses
    std::set<CSecretAddress>::iterator it;
    for (it = secretAddresses.begin(); it != secretAddresses.end(); ++it)
    {
        if (it->scan_secret.size() < 32)
        ...
        if (fDebug)
            printf("Decrypting secret key %s\n", sxAddr.Encoded().c_str());
        ...
        if(!DecryptSecret(vMasterKeyIn, sxAddr.spend_secret, iv, vchSecret)
            || vchSecret.size() != 32)
        {
            printf("Error: Failed decrypting secret key %s\n", sxAddr.Encoded().c_str());
            continue;
        };
        ...
        {
            printf("Error: Failed decrypting secret key, public key mismatch %s\n", sxAddr.Encoded().c_str());
            continue;
        };
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        SecretKeyMetaMap::iterator mi = mapSecretKeyMeta.find(ckid);
        if (mi == mapSecretKeyMeta.end())
        {
            printf("Error: No metadata found to add secret for %s\n", addr.ToString().c_str());
            continue;
        };
        
        CSecretKeyMetadata& sxKeyMeta = mi->second;
        
        CSecretAddress sxFind;
        sxFind.scan_pubkey = sxKeyMeta.pkScan.Raw();
        
        std::set<CSecretAddress>::iterator si = secretAddresses.find(sxFind);
        if (si == secretAddresses.end())
        {
            printf("No secret key found to add secret for %s\n", addr.ToString().c_str());
            continue;
        };
        ...
        {
            printf("Secret address has no secret key for %s\n", addr.ToString().c_str());
            continue;
        }
        ...
        if (SecretSecretSpend(sScan, pkEphem, sSpend, sSpendR) != 0)
        {
            printf("SecretSecretSpend() failed.\n");
            continue;
        };
        ...
        if (!CWalletDB(strWalletFile).EraseSecretKeyMeta(ckid))
            printf("EraseSecretKeyMeta failed for %s\n", addr.ToString().c_str());
    };
    return true;
}

bool CWallet::UpdateSecretAddress(std::string &addr, std::string &label, bool addIfNotExist)
{
    if (fDebug)
        printf("UpdateSecretAddress %s\n", addr.c_str());
    
    
    CSecretAddress sxAddr;
    
    if (!sxAddr.SetEncoded(addr))
        return false;
    
    std::set<CSecretAddress>::iterator it;
    it = secretAddresses.find(sxAddr);
    
    ChangeType nMode = CT_UPDATED;
    CSecretAddress sxFound;
    if (it == secretAddresses.end())
    {
        if (addIfNotExist)
        {
            sxFound = sxAddr;
            sxFound.label = label;
            secretAddresses.insert(sxFound);
            nMode = CT_NEW;
        } else
        {
            printf("UpdateSecretAddress %s, not in set\n", addr.c_str());
            return false;
        };
    } else
    {
        sxFound = const_cast<CSecretAddress&>(*it);
        ...
        it->label = label; // update in .secretAddresses
        
        if (sxFound.scan_secret.size() == ec_secret_size)
        {
            printf("UpdateSecretAddress: todo - update owned secret address.\n");
            return false;
        };
    };
    ...
    if (!CWalletDB(strWalletFile).WriteSecretAddress(sxFound))
    {
        printf("UpdateSecretAddress(%s) Write to db failed.\n", addr.c_str());
        return false;
    };
...
bool CWallet::CreateSecretTransaction(CScript scriptPubKey, int64_t nValue, std::vector<uint8_t>& P, std::vector<uint8_t>& narr, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, std::string strDZeel, const CCoinControl* coinControl)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    ...
    if (rv && narr.size() > 0)
    {
        for (unsigned int k = 0; k < wtxNew.vout.size(); ++k)
        {
            ...
            {
                printf("CreateSecretTransaction(): Error creating narration key.");
                break;
            };
...
string CWallet::SendSecretMoney(CScript scriptPubKey, int64_t nValue, std::vector<uint8_t>& P, std::vector<uint8_t>& narr, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee, std::string strDZeel)
{
    CReserveKey reservekey(this);
    ...
    if (IsLocked())
    {
        ...
        printf("SendSecretMoney() : %s", strError.c_str());
        return strError;
    }
    ...
    if (!CreateSecretTransaction(scriptPubKey, nValue, P, narr, sNarr, wtxNew, reservekey, nFeeRequired, strDZeel))
    {
        string strError;
...
bool CWallet::SendSecretMoneyToDestination(CSecretAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee, std::string strDZeel)
{
    // -- Check amount
    if (nValue <= 0)
    {
    ...
    if (SecretSecret(ephem_secret, sxAddress.scan_pubkey, sxAddress.spend_pubkey, secretShared, pkSendTo) != 0)
    {
    ...
    if (fDebug)
    {
        printf("Secret send to generated pubkey %" PRIszu": %s\n", pkSendTo.size(), HexStr(pkSendTo).c_str());
    ...
    if ((sError = SendSecretMoney(scriptPubKey, nValue, ephem_pubkey, vchNarr, sNarr, wtxNew, fAskFee, strDZeel)) != "")
        return false;
...
bool CWallet::FindSecretTransactions(const CTransaction& tx, mapValue_t& mapNarr)
{
    if (fDebug)
        LogPrintf("FindSecretTransactions() tx: %s\n", tx.GetHash().GetHex().c_str());
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
            ...
            || vchEphemPK.size() != 33)
        {
                ...
                && vchEphemPK[1] == 'p')
            {
                ...
                {
                    printf("Warning: FindSecretTransactions() tx: %s, Could not extract plaintext narration.\n", tx.GetHash().GetHex().c_str());
                };
            }
        ...
        BOOST_FOREACH(const CTxOut& txoutB, tx.vout)
        {
            ...
            for (it = secretAddresses.begin(); it != secretAddresses.end(); ++it)
            {
                ...
                if (SecretSecret(sScan, vchEphemPK, it->spend_pubkey, sShared, pkExtracted) != 0)
                {
                    printf("SecretSecret failed.\n");
                    continue;
                };
                ...
                if (fDebug)
                    printf("Found secret txn to address %s\n", it->Encoded().c_str());
                ...
                if (IsLocked())
                {
                    ...
                    CSecretKeyMetadata lockedSkMeta(cpkEphem, cpkScan);
                    
                    if (!CWalletDB(strWalletFile).WriteSecretKeyMeta(keyId, lockedSkMeta))
                        printf("WriteSecretKeyMeta failed for %s\n", coinAddress.ToString().c_str());
                    
                    mapSecretKeyMeta[keyId] = lockedSkMeta;
                    nFoundSecret++;
                } else
                {
                    ...
                    if (SecretSharedToSecretSpend(sShared, sSpend, sSpendR) != 0)
                    {
                        printf("SecretSharedToSecretSpend() failed.\n");
                        continue;
                    };
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    bool bINodePayment = true; // note was false, set true to test

    if ( Params().NetworkID() == CChainParams::TESTNET ){
        if (GetTime() > START_INODE_PAYMENTS_TESTNET ){
            bINodePayment = true;
        }
    }else{
        if (GetTime() > START_INODE_PAYMENTS){
            bINodePayment = true;
        }
    }
    ...
    int64_t inodePayment = GetInodePayment(pindexPrev->nHeight+1, nReward);
...
{
    mapValue_t mapNarr;
    FindSecretTransactions(wtxNew, mapNarr);
...
string CWallet::SendMoney(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee, std::string strDZeel)
{
    ...
    if (!CreateTransaction(scriptPubKey, nValue, sNarr, wtxNew, reservekey, nFeeRequired, strDZeel))
    {
        string strError;
        if (nValue + nFeeRequired > GetBalance())
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee, std::string strDZeel)
{
    // Check amount
    if (nValue <= 0)
    ...
    return SendMoney(scriptPubKey, nValue, sNarr, wtxNew, fAskFee, strDZeel);
}


string CWallet::PrepareAnonsendDenominate(int minRounds, int maxRounds)
{
    if (IsLocked())
    ...
    if(anonSendPool.GetState() != POOL_STATUS_ERROR && anonSendPool.GetState() != POOL_STATUS_SUCCESS)
        if(anonSendPool.GetMyTransactionCount() > 0)
            return _("Error: You already have pending entries in the Anonsend pool");
    ...
    */
    if(minRounds >= 0){
        if (!SelectCoinsByDenominations(anonSendPool.sessionDenom, 0.1*COIN, ANONSEND_POOL_MAX, vCoins, vCoins2, nValueIn, minRounds, maxRounds))
            return _("Insufficient funds");
    }

    // calculate total value out
    int64_t nTotalValue = GetTotalValue(vCoins);
    LogPrintf("PrepareAnonsendDenominate - preparing anonsend denominate . Got: %d \n", nTotalValue);
    ...
    */
    ...
    std::random_shuffle (vDenoms.begin() + (int)anonSendDenominations.size() + 1, vDenoms.end());

    // Make outputs by looping through denominations randomly
    BOOST_REVERSE_FOREACH(int64_t v, vDenoms){
        //only use the ones that are approved
        bool fAccepted = false;
        if((anonSendPool.sessionDenom & (1 << 0))      && v == ((100000*COIN) +100000000)) {fAccepted = true;}
        else if((anonSendPool.sessionDenom & (1 << 1)) && v == ((10000*COIN)  +10000000)) {fAccepted = true;}
        else if((anonSendPool.sessionDenom & (1 << 2)) && v == ((1000*COIN)   +1000000)) {fAccepted = true;}
        else if((anonSendPool.sessionDenom & (1 << 3)) && v == ((100*COIN)    +100000)) {fAccepted = true;}
        else if((anonSendPool.sessionDenom & (1 << 4)) && v == ((10*COIN)     +10000)) {fAccepted = true;}
        else if((anonSendPool.sessionDenom & (1 << 5)) && v == ((1*COIN)      +1000)) {fAccepted = true;}
        else if((anonSendPool.sessionDenom & (1 << 6)) && v == ((.1*COIN)     +100)) {fAccepted = true;}
        if(!fAccepted) continue;
    ...
    if(vOut.size() > 40 || anonSendPool.GetDenominations(vOut) != anonSendPool.sessionDenom || nValueLeft != 0){
        vOut.clear();
    ...
    if(anonSendPool.GetDenominations(vOut) != anonSendPool.sessionDenom)
    ...
    anonSendPool.SendAnonsendDenominate(vCoins, vOut, nValueIn);
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s NAV %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s NAV %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
{
    if(!IsHubActive(HUB_1_INODE_PAYMENTS_ENFORCEMENT)) return -3;
    if(nTesseractXDepth == 0) return -1;
...
{
    if(nTesseractXDepth == 0) return 0;
...
bool CWallet::AddRushNodeConfig(CRushNodeConfig nodeConfig)
{
    bool rv = CWalletDB(strWalletFile).WriteRushNodeConfig(nodeConfig.sAlias, nodeConfig);
    if(rv)
	uiInterface.NotifyRushNodeChanged(nodeConfig);
