...
static const int64_t ANONSEND_COLLATERAL = (100000*COIN);
static const int64_t ANONSEND_FEE = (0.0001*COIN);
static const int64_t ANONSEND_POOL_MAX = (1000000.99*COIN);

/*
    At 15 signatures, 1/2 of the inode network can be owned by
    one party without comprimising the security of TesseractX
    (1000/2150.0)**15 = 1.031e-05
*/
#define TESSERACTX_SIGNATURES_REQUIRED           20
#define TESSERACTX_SIGNATURES_TOTAL              30
...
static const unsigned int MAX_DZEEL = 528;
static const unsigned int DZEEL_HEIGHT = 2;
...
static const int64_t MAX_MONEY = 60000000 * COIN; // 1M PoW coins
...
int64_t GetInodePayment(int nHeight, int64_t blockValue);
...
public:
	...
	static const int TXDZEEL_VERSION=2;
    int nVersion;
    unsigned int nTime;
    ...
    (
        ...
        READWRITE(vout);
        READWRITE(nLockTime);
        if(this->nVersion >= TXDZEEL_VERSION) { 
        READWRITE(strDZeel); }
    )
